###
# overlap DE genes and DM in CCGG context, 5hmC
###
...
for (feature in Feature){print(feature)
  ...
  Catalog = paste("DMR_CCGG_5hmC_", feature, sep = "")
  ...
  for (group in c("Neil1", "Neil2", "DK")) {print(group)
    ...
    df_5hmC_hyper$mC_type <- "hyper"
    
    # hypo
    inFile = paste(driver, "out/data/table_", group, "_vs_WT_5hmC_", pattern, "_imputedWith_zeros_DMRs_hypo.csv", sep = "")
    df_5hmC_hypo <- read.delim(file = inFile, header = T, strip.white = T, sep = "\t")  # strip white space in gene column ! 
    df_5hmC_hypo$mC_type <- "hypo"
    
    # rbind
    df_5hmC <- rbind(df_5hmC_hyper, df_5hmC_hypo)
    df_5hmC$catalog <- Catalog
    df_5hmC$mC_dif <- df_5hmC$median_KO - df_5hmC$median_WT
    Location <- substring(df_5hmC$location, 4, 300)
    df_5hmC$location <- Location
    df_5hmC$distance_to_gene <- 0
    ...
    # pattern in 5hmC_hyper
    mC_data <- df_5hmC %>% arrange(gene) %>% distinct(gene, .keep_all =T)
