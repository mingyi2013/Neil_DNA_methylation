...
# gene annotation organism: mouse
organism = org.Mm.eg.db 

# set context
Context = c("CG", "CHG", "CHH")

# input common_Genes_C*.txt:
#read.delim("./dataIn_fig4/commonGenes_CG.txt")  # test
# set input data path:
Path = "./dataIn_fig4/"

for (context in Context) { print(context)
File_in <- paste(Path, "commonGenes_", context, ".txt", sep = "")
data <- read.delim(file = File_in) %>% 
        mutate(mean_dif = (mean_dif.x + mean_dif.y + mean_dif.z)/3)
     
    # create out fold  
    out =  paste("GO_commonGenes_",context, sep = "")
    dir.create(out)   
    ...
    # we want the log2_fold_change (here mean_dif !!) 
    ...
    if (context == "CG") {
      Cutoff = 0.4
      cat("set pavlue and qvalue Cutoff as: ", Cutoff, "for", context, "\n")
    } else{
      Cutoff = 0.01
      cat("set pavlue and qvalue Cutoff as: ", Cutoff, "for", context, "\n")
    }
    ...
    for (j in type) {print(j)
      ...
      gene <- names(kegg_gene_list)[abs(kegg_gene_list) > 0.06]
    ...
    } # end of for_loop in BP, MF and CC
