#############
# ggplot for 5hmC profile
#############
library(tidyverse)

### step1: ggplot for each sample
#setwd("~/project_katja2/hmst_5hmC/")
#tss1 <- read.csv("~/project_katja2/hmst_5hmC/out_chrAll/plots/plotData/plotData_TSSgeneTES_smoothed_5hmC_allMRs.csv", header=T) #original
tss1 <- read.csv("./dataIn_fig5/out_chrAll/plots/plotData/plotData_TSSgeneTES_smoothed_5hmC_allMRs.csv", header=T)
head(tss1,3)
as.tibble(tss1)
...
ggplot(tss1) +
  ...
  labs( y = "Average methylation Level", x = " ", title = "5hmC profile") +
...
dev.copy(jpeg, file="ggplot_5hmC_profile.jpg",
         units="in", 
         width=6, 
         height=4, 
         res=300)
...
# boxplot for Figure 5B
boxplot(data1_tgt,
        main = "5hmC in TSS_gene_TES",
        xlab = "",
...
boxplot(data2_tss,
        ...
        ylab = "Average Methylation Level",
        ylim = c(0.5, 1.4) ) 
dev.copy(jpeg, file="boxplot_6M_5hmC_TSS.jpg",
         units="in", 
         width=4, 
         height=4, 
         res=300)
...
boxplot(data3_gene,
        main = "5hmC in gene_body",
        xlab = "",
        ylab = "Average Methylation Level",
        ylim = c(1.0, 1.6) )  
dev.copy(jpeg, file="boxplot_6M_5hmC_geneBody.jpg",
         units="in", 
         width=4, 
         height=4, 
         res=300)
...
boxplot(data4_tes,
        ...
        ylab = "Average Methylation Level",
        ylim = c(1.0, 1.6) )  
dev.copy(jpeg, file="boxplot_6M_5hmC_TES.jpg",
         units="in", 
         width=4, 
         height=4, 
         res=300)
...
## two sided test, all are sig, except TSS for DK_vs_WT_10d.
